...
let lastPlacesRequestAt = 0;

async function respectNominatimThrottle() {
  const wait = Math.max(0, 1_000 - (Date.now() - lastPlacesRequestAt));
  if (wait > 0) await new Promise((resolve) => setTimeout(resolve, wait));
  lastPlacesRequestAt = Date.now();
}
...
export class DemoPlacesProvider implements PlacesProvider {
  async search(input: { category?: PlaceCategory; query?: string; origin?: BrowserLocation }): Promise<PlaceResult[]> {
    ...
    if (input.origin) {
      await respectNominatimThrottle();
...
function place(id: string, name: string, category: PlaceCategory, distanceKm: number, travelMinutes: number, extra: Partial<PlaceResult> = {}, source: SourceStatus = 'local'): PlaceResult {
  return { id, name, category, address: extra.address ?? 'Nähe aktueller Position', distanceKm, travelMinutes, rating: extra.rating ?? 4.3, openNow: extra.openNow ?? true, latitude: extra.latitude ?? 52.52 + distanceKm / 100, longitude: extra.longitude ?? 13.405 + distanceKm / 120, source };
}

export const demoPlaces: PlaceResult[] = [
  place('fuel-1', 'Tankstelle Nord', 'fuel', 1.1, 4, { rating: 4.1 }),
  place('fuel-2', 'Stadttankstelle', 'fuel', 2.8, 6, { rating: 4.4 }),
  place('fuel-3', 'Service Station West', 'fuel', 3.4, 9, { rating: 4.0 }),
  place('market-1', 'Supermarkt Zentrum', 'supermarket', 0.9, 3, { rating: 4.5 }),
  place('pharmacy-1', 'Apotheke am Markt', 'pharmacy', 1.4, 5, { rating: 4.7 }),
  place('parking-1', 'Parkdeck Zentrum', 'parking', 0.6, 2, { rating: 4.0 }),
  place('workshop-1', 'Kfz-Werkstatt West', 'workshop', 3.1, 8, { rating: 4.6 }),
  place('food-1', 'Restaurant City', 'restaurant', 1.8, 5, { rating: 4.0 }),
  place('cloth-1', 'Modehaus Innenstadt', 'clothing', 2.2, 7, { rating: 4.2 }),
  place('atm-1', 'Geldautomat Zentrum', 'atm', 0.7, 2, { rating: 4.3 }),
  place('parcel-1', 'Paketstation 144', 'parcel', 1.2, 4, { rating: 4.1 }),
  place('charge-1', 'E-Ladestation West', 'charging', 2.5, 6, { rating: 4.4 }),
];

export function bestFuelRecommendation(places: PlaceResult[]): PlaceResult | undefined {
  return places.filter((place) => place.category === 'fuel').sort((a, b) => a.distanceKm - b.distanceKm)[0];
}
